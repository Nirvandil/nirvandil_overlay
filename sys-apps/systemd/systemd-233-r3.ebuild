...
IUSE="acl apparmor audit build cryptsetup curl doc elfutils +gcrypt gnuefi http
	idn -ima importd +kmod +lz4 lzma nat pam policykit
	qrcode +seccomp selinux -smack ssl sysv-utils test vanilla xkb"
...
pkg_pretend() {
	...
	use importd && CONFIG_CHECK+=" ~VETH"
	use seccomp && CONFIG_CHECK+=" ~SECCOMP ~SECCOMP_FILTER"
	# By Nirvandil 
	use ima && CONFIG_CHECK+=" ~IMA"
	use smack && CONFIG_CHECK+=" ~SECURITY_SMACK"
