...
SRC_URI="
	amd64? ( ${SRC_BASE}-x86_64.tar.gz&r=1 -> ${PN}-x86_64.tar.gz )
	x86? ( ${SRC_BASE}.tar.gz&r=1 -> ${PN}.tar.gz )
"
...
src_install() {
	...
	cp "${FILESDIR}"/eclipserc-bin-${SLOT} "${T}" || die "no eclipserc"
	cp "${FILESDIR}"/eclipse-bin-${SLOT} "${T}" || die	"no eclipse-bin"
	sed "s@%SLOT%@${SLOT}@" -i "${T}"/eclipse{,rc}-bin-${SLOT} || die "sed for SLOT failed"
	...
	make_desktop_entry "eclipse-cpp-${SLOT}" "Eclipse ${RNAME}-${SR}" "${dest}/icon.xpm" "Development;IDE"
