...
DESCRIPTION="NoSQL database with scalability and HA without compromising performance"
...
src_install() {
	...
	if use systemd; then
		sed -e "s/{SLOT}/${SLOT}/g" -e "s/{PV}/${PV}/g" "${FILESDIR}/cassandra.service" > \
			"${T}/cassandra-${SLOT}.service" || die
